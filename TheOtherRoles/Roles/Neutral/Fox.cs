        ...
        public override void FixedUpdate()
        {
            if (player == PlayerControl.LocalPlayer)
            {
                ...
                if (player.isAlive())
                {
                    ...
                    {
                        if (p.isImpostor() || p.isRole(RoleType.Jackal) || p.isRole(RoleType.Sheriff) || p.isRole(RoleType.Boss) || p.isRole(RoleType.Staff) || p.isRole(RoleType.Gun))
                        {
        ...
        static void arrowUpdate()
        {
            ...
            if (updateTimer <= 0.0f)
            {
                ...
                foreach (PlayerControl p in PlayerControl.AllPlayerControls)
                {
                    ...
                    if (p.Data.Role.IsImpostor || p.isRole(RoleType.Jackal) || p.isRole(RoleType.Sheriff)/* || p.isRole(RoleType.Boss) || p.isRole(RoleType.Staff) || p.isRole(RoleType.Gun)*/)
                    {
                        ...
                        else if (p.isRole(RoleType.Boss))
                        {
                            arrow = new Arrow(Boss.color);
                        }
                        else if (p.isRole(RoleType.Staff))
                        {
                            arrow = new Arrow(Boss.color);
                        }
                        else if (p.isRole(RoleType.Gun))
                        {
                            arrow = new Arrow(Boss.color);
                        }
                        else
