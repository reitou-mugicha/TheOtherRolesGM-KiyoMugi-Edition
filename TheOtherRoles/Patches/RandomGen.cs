...
ありがとうございます by Mugicha
誠ににありがとうございます by 𝑫𝒆𝒌𝒐𝑲𝒊𝒚𝒐
*/

namespace TheOtherRoles.Patches
{
    public static class RandomGeneratorPatch
    {
        public static PermutedCongruentialGenerator Instance;
        public static bool prevValue = false;
        public static bool improvementRandom { get { return CustomOptionHolder.randomGen.getBool(); } }

        public static void Initialize()
        {
            if (Instance == null)
            {
                createGlobalRandomGenerator(improvementRandom);
            }
            else
            {
                if (improvementRandom != prevValue)
                {
                    createGlobalRandomGenerator(improvementRandom);
                }
            }
