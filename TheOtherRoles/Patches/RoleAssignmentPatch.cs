        ...
        private static void selectFactionForFactionIndependentRoles(RoleAssignmentData data)
        {
            ...
            if (CustomOptionHolder.mayorSpawnBothRate.getSelection() > 0) {
                if (rnd.Next(1, 101) <= CustomOptionHolder.mayorSpawnRate.getSelection() * 10) {
                    if (isEvilGuesser) {
                        if (data.impostors.Count > 0 && data.maxImpostorRoles > 0) {
                            byte evilMayor = setRoleToRandomPlayer((byte)RoleType.EvilMayor, data.impostors);
                            data.impostors.ToList().RemoveAll(x => x.PlayerId == evilMayor);
                            data.maxImpostorRoles--;
                            data.crewSettings.Add((byte)RoleType.Mayor, (CustomOptionHolder.mayorSpawnBothRate.getSelection(), 1));
                        }
                    }
                    else if (data.crewmates.Count > 0 && data.maxCrewmateRoles > 0) {                    
                        byte mayor = setRoleToRandomPlayer((byte)RoleType.Mayor, data.crewmates);
                        data.crewmates.ToList().RemoveAll(x => x.PlayerId == mayor);
                        data.maxCrewmateRoles--;
                        data.impSettings.Add((byte)RoleType.EvilMayor, (CustomOptionHolder.mayorSpawnBothRate.getSelection(), 1));
                    }
                }
            } else {
